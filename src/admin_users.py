...
def buscar_assinatura_usuario(usuario_id):
    with conectar_banco() as conexao:
...
def renderizar_assinatura_pagamentos(
    usuario_id
):
    st.markdown(
        "### Assinatura"
    ...
    assinatura = buscar_assinatura_usuario(
        usuario_id
    )
    ...
    with st.container(
        ...
        if not assinatura:
            ...
            st.markdown(
                "**Plano:** "
                f"{formatar_campo(assinatura['plano_codigo'])}"
            )

            st.markdown(
                "**Provedor:** "
                f"{formatar_campo(assinatura['provedor'])}"
            )

            st.markdown(
                "**ID externo:** "
                f"{formatar_campo(assinatura['assinatura_externa_id'])}"
            )

            st.markdown(
                "**Início:** "
                f"{formatar_campo(assinatura['inicio_em'])}"
            )

            st.markdown(
                "**Próxima cobrança:** "
                f"{formatar_campo(assinatura['proxima_cobranca_em'])}"
            )

            st.markdown(
                "**Cancelado em:** "
                f"{formatar_campo(assinatura['cancelado_em'])}"
            )
...
def renderizar_edicao_usuario(usuarios):
    ...
    renderizar_assinatura_pagamentos(
        usuario_id
    )
